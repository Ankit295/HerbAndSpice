...
all=[data.array[0].id,data.array[1].id,data.array[2].id,data.array[3].id,data.array[4].id,data.array[5].id,data.array[6].id,data.array[7].id,data.array[8].id];
        for(var j=0;j<9;j++){
            ...
            $('#content').append("<a name='"+idc+"'></a><div id='"+idc+"' class='cat' style=\"background:url('menu/"+idc+".jpg') no-repeat #E7E7E7;background-size: contain;\"></div>");
            elem = $('#'+idc);
       ...
       "<div> <img class='pimg' src='menu/"+id+".jpg'></img></div>"+
       "<div class='pdet'>"+
       ...
       "<div class='actions'><a href='javascript:addtocart(\""+id+"\" , \""+dname+"\" , \""+cost+"\")' class='addtocart'><span>add to order</span><img class='cart-icon' src='menu/cart.svg'></img></a></div><div style='clear: both'></div>";
...
function addtocart(id, name, price){
  ...
  console.log(nos);
  $.ajax({
  url: "http://localhost:3000/addcart/"+id+'/'+name+'/'+price+'/'+nos,
  dataType: "jsonp",
  jsonpCallback: 'popup',
  type:'GET',
    data: {
...
function popup(data)
{
    ...
    var name = data.name;
    var item=$('#addpopup');
    $('#addpopup img').attr('src','menu/'+id+'.jpg');
  ...
  function updateIt(){
  $.ajax({
  url: "http://localhost:3000/getactiveuser",
  dataType: "jsonp",
...
function viewCart(){
  ...
  var str= '<div class="popcart-indata">'+
    '<img width ="50px"; height="50px" src="menu/'+bigCart.array[0].order[i].id+'.jpg" >'+
...
function finishJob(){
  $.ajax({
  url: "http://localhost:3000/pushorder",
  dataType: "jsonp",
